    ...
    end_behavior(WaypointNavigationGoal::HOVER)
{
    ...
    if (!pnh.getParam("costmap_frame", costmap_frame)) {
        ROS_WARN("[WaypointNavigationVel] failed to fetch parameter \"costmap_frame\" "
                 "using default value of \"map\"");
        world_frame = "map";
    }
    if (!pnh.getParam("world_frame", world_frame)) {
        ROS_WARN("[WaypointNavigationVel] failed to fetch parameter \"world_frame\" "
                 "using default value of \"map\"");
        world_frame = "world";
    }
    if (!pnh.getParam("local_frame", local_frame)) {
        ROS_WARN("[WaypointNavigationVel] failed to fetch parameter \"local_frame\" "
                 "using default value of \"map\"");
        local_frame = "map";
    }
    ...
    if (!pnh.getParam("position_tol", position_tol)) {
        position_tol = 0.2;
        ROS_WARN("[WaypointNavigationVel] failed to fetch parameter \"position_tol\" "
                 "using default value of %.2fm", position_tol);
    }
    if (!pnh.getParam("heading_tol", heading_tol)) {
        heading_tol = 0.2;
        ROS_WARN("[WaypointNavigationVel] failed to fetch parameter \"heading_tol\" "
...
void WaypointNavigationVel::costmapCB(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
    ...
    if (!obstacle_free) {
        //ROS_INFO("[WaypointNavigationVel] collision detected! replanning path");
    ...
    if (g_client.call(get_req, get_resp)) {
        if (get_resp.gravity.z != 0.0) {
            ...
            if (s_client.call(set_req, set_resp)) {
                ROS_INFO_STREAM("Successfully set gravity to 0");
            } else {
                ROS_INFO_STREAM("Failed to set gravity to 0");
            }
        }
    } else {
        ROS_INFO_STREAM("Failed to get physics properties.");
    }
...
void WaypointNavigationVel::planPath(const geometry_msgs::PoseStamped& robot_pose)
{
    ...
    if (!costmap.inBounds(robot_pose)) {
        ROS_WARN("[WaypointNavigationVel] odom out of bounds of costmap: setting "
    ...
    for (auto& pt : waypoints) {
        if (!costmap.inBounds(pt)) {
            ROS_INFO("[WaypointNavigationVel] waypoint out of bounds of costmap: "
                     "expanding costmap");
