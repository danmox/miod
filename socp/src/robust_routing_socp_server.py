...
class ChannelModel:
    def __init__(self, fetch_params=False, print_values=True, n0=-70.0, n=2.52, l0=-53.0, a=0.2, b=6.0):
        if fetch_params is False:
...
class RobustRoutingSolver:
    def __init__(self, fetch_params=False, print_values=True, n0=-70.0, n=2.52, l0=-53.0, a=0.2, b=6.0):
        self.cm = ChannelModel(fetch_params, print_values, n0, n, l0, a, b)

    # qos - a list of qos dicts
    # x   - a 2xN np array of x,y positions
    def rrsocpconf(self, qos, x):

        # node margin constraint coefficients
    ...
    def solve_socp(self, req):
        ...
        slack, routes, status = self.rrsocpconf(req.qos, x)
        if status == 'optimal':
            res.slack = slack
            res.routes = routes
...
def robust_routing_socp_server():
    ...
    rrsolver = RobustRoutingSolver(fetch_params=True, print_values=True)
    s = rospy.Service('RobustRoutingSOCP', RobustRoutingSOCP, rrsolver.solve_socp)
