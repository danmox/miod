...
{
  ros::init(argc, argv, "channel_visualization_node");
  ...
  if (!nh.getParam("/task_agent_count", task_agent_count) ||
    ...
    ROS_FATAL("[channel_visualization_node] failed to fetch parameter(s) from server");
    return -1;
  }
  ...
  ros::Rate rate(10);
  ROS_INFO("[channel_visualization_node] starting loop");
  while (ros::ok()) {
    rate.sleep();
    ros::spinOnce();
    ...
    for (int i = 1; i <= number_of_agents; ++i) {
      ...
      try {
        ...
        ROS_WARN("[channel_visualization_node] %s", ex.what());
        publish_viz = false;
        break;
      }
    ...
    if (publish_viz) {
      ...
      for (auto& pair : perms) {
        ...
        color.a = pair.rate;
        ROS_DEBUG("[channel_visualization_node] alpha = %.2f", pair.rate);
      ...
      ROS_WARN("[channel_visualization_node] not publishing this iteration");
    }
  }
